...
impl AssociatedItem {
    pub fn kind(&self) -> AssociatedItemKind {
        match self.syntax.kind() {
            FUNCTION_DEF => AssociatedItemKind::FunctionDef(FunctionDef::cast(self.syntax.clone()).unwrap()),
            _ => unreachable!(),
        }
    }
}
...
impl AstNode for Expr {
    fn can_cast(kind: SyntaxKind) -> bool {
        matches!(kind,
             LITERAL | PREFIX_EXPR | PATH_EXPR | BIN_EXPR | PAREN_EXPR | CALL_EXPR | FIELD_EXPR | IF_EXPR | LOOP_EXPR | WHILE_EXPR | RETURN_EXPR | BREAK_EXPR | BLOCK_EXPR | ARRAY_EXPR | INDEX_EXPR | RECORD_LIT
        )
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        if Self::can_cast(syntax.kind()) {
...
impl AstNode for TypeRef {
    fn can_cast(kind: SyntaxKind) -> bool {
        matches!(kind,
             PATH_TYPE | ARRAY_TYPE | NEVER_TYPE
        )
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        if Self::can_cast(syntax.kind()) {
