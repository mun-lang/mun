...
impl RuntimeBuilder {
    ...
    pub fn spawn(self) -> Result<Rc<RefCell<Runtime>>, anyhow::Error> {
...
impl Runtime {
    ...
    /// of `dur`.
    pub fn new(mut options: RuntimeOptions) -> Result<Runtime, anyhow::Error> {
        let (tx, rx) = channel();
    ...
    fn add_assembly(&mut self, library_path: &Path) -> Result<(), anyhow::Error> {
