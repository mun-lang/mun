...
LldInvokeResult mun_lld_link(LldFlavor flavor, int argc, const char *const *argv) {
  ...
  switch(flavor)
  {
    case Elf:
    {
      ...
      result.success = lld::elf::link(args, false, errorStream, errorStream);
      break;
    }
    case Wasm:
    {
      std::unique_lock<std::mutex> lock(_wasmMutex);
      result.success = lld::wasm::link(args, false, errorStream, errorStream);
      break;
    }
    case MachO:
    {
      std::unique_lock <std::mutex> lock(_machOMutex);
      result.success = lld::mach_o::link(args, false, errorStream, errorStream);
      break;
    }
    case Coff:
    {
      ...
      result.success = lld::coff::link(args, false, errorStream, errorStream);
      break;
    }
    default:
